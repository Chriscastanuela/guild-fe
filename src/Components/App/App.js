...
export default class App extends Component {
  ...
  async componentDidMount() {
    console.log(await requests.getStudents(), await requests.getCourses());
  ...
  postStudent = async (first, last, email) => {
    ...
    if (answer.error) {
      ...
      this.setState({
        postStudentResponse: `Registered ${answer.firstName} with ID ${answer.id}. Please keep your ID to log in.`,
      });
    }
    return answer;
  };
        ...
        <div className="app">
          <NavLink to="/" className="links" id="header-link">
            <section className="header">
              <em>
                <h1 className="header-title" id="header-main">
                  School Registration
                </h1>
              </em>
